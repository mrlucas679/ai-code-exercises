...
/** Creates a new task, persists it, and returns its id. Returns null if date is invalid. */
  createTask(title, description = "", priorityValue = 2, dueDateStr = null, tags = []) {
    ...
    const task = new Task(title, description, priorityValue, dueDate, tags);
...
/** Updates a task's status. Calls markAsDone() when status is DONE to set completedAt. */
  ...
  /** Adds a tag to a task. Returns true if the task was found (tag may or may not have been new), false if task not found. */
  ...
  /** Marks tasks as ABANDONED if overdue for more than 7 days and not high priority or urgent.
   * Returns the number of tasks abandoned.
   */
  autoAbandonOverdueTasks() {
    const tasks = this.storage.getAllTasks();
    const sevenDaysAgo = new Date();
    sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
    let count = 0;
    ...
    tasks.forEach(task => {
      const isLongOverdue = task.dueDate && task.dueDate < sevenDaysAgo;
      const isHighPriority = task.priority >= TaskPriority.HIGH;
      const isActive = task.status !== TaskStatus.DONE && task.status !== TaskStatus.ABANDONED;

      if (isLongOverdue && !isHighPriority && isActive) {
        task.markAsAbandoned();
        count++;
      }
    });

    if (count > 0) this.storage.save();
    return count;
  }
}
